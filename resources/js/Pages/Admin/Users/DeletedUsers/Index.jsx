...
const Index = ({ auth, title, users }) => {
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        router.get(route("users.index"), { s: search });
    };
                    ...
                    <div className="card-box table-responsive">
                        <div className="row">
                            <div className="col-sm-3">
                                ...
                                >
                                    ...
                                    <option value="?plan_id=1">Basic Plan</option>
                                    <option value="?plan_id=2">Premium Plan</option>
                                    <option value="?plan_id=3">Platinum Plan</option>
                                    <option value="?plan_id=4">Diamond Plan</option>
                                </select>
                            </div>
                            ...
                            <div className="col-md-3">
                                <Link
                                    ...
                                    title="Export Users"
                                >
                                    <i className="fa fa-file-excel-o"></i> Export Users
                                </Link>
                            </div>
                        </div>
                        ...
                        <div className="table-responsive">
                            <table className="table table-bordered">
                                ...
                                <tbody>
                                    {users.data.map((user) => (
                                        <tr key={user.id} id={`card_box_id_${user.id}`}>
                                            <td>{user.name}</td>
                                            <td>{user.email}</td>
                                            <td>{user.phone || '-'}</td>
                                            <td>
                                                <span className={`badge badge-${user.status === 1 ? 'success' : 'danger'}`}>
