...
const Create = ({ auth, errors, director }) => {
    ...
    useEffect(() => {
        if (director && director.image_urls) {
            setImagePreviews(director.image_urls);
        }
    }, [director]);
    ...
    const handleSubmit = async (e) => {
        ...
        const msg = director ? 'Director updated successfully.' : 'Director created successfully.';
        
        await router.post(route("admin.directors.save", encryptString(data?.id)), // Pass encrypted ID directly
            { ...data },
            { onSuccess: () => {
    ...
    return (
        <Authenticated user={auth.user}>
            <Head title={director ? "Edit Director" : "Add Director"} />
                        ...
                        {director?.images?.length > 0 ? (
                            director.images.map((image, index) => (
                                <MyImage 
                                    ...
                                    altText={`Image for ${director.name}`}
                                    class="img-thumbnail mr-1" 
                                    style={{width: '180px'}}
                                />
                            ))
                        ) : (
                            <p className="offset-sm-2">No images available for this director.</p>
                        )}
                    </Form.Group>
